...
# Empirically verified against 49 real reports / 126,629 score rows
# (kmcken/CompetitionShootingAnalytics Data/txtFiles corpus):
#   * field 3  = per-stage DQ. All 3,057 "Yes" rows had hf == 0 and belonged
#     to shooters whose E-line DQ flag (field 4) was set — 100% overlap.
#   * field 4  is NOT DNF (the reference parser's mapping is wrong): it is
#     "Yes" on ~60% of perfectly valid scored rows and is near-constant
#     within each file — a per-match app setting. It is ignored here, and
#     DNF is derived instead: no time, no points, no hit factor.
#   * fields 25/27/28: hit_factor == total_points / time to 4 decimals in
#     100.0% of rows with time > 0 (115,110 / 115,156 exact).
I_STAGE, I_COMP, I_DQ = 1, 2, 3
    ...
    for raw_line in lines:
        try:
            if raw_line.startswith("E "):
                ...
                f = raw_line[2:].rstrip("\n").split(",")
                # Repair: a comma inside the stage name shifts columns.
                # Real G lines have exactly 9 fields (name is field 6 of 9),
                # so fold any extras back into the name.
                if len(f) > 9:
                    f = f[:6] + [",".join(f[6:len(f) - 2])] + f[-2:]
                if len(f) <= G_SCORING:
                ...
                # DNF derived, not read: no time, no points, no HF.
                # (Field 4 looks like a DNF flag but is a per-match app
                # setting — see the field-position notes above.)
                dnf = (not time_s) and (not total_pts) and (not hf)
                scores.append({
...
def report_region(text: str) -> str | None:
    """`$INFO Region:` value — USPSA, IPSC, SCSA, ... None if absent."""
