...
def main() -> None:
    ...
    if reg.empty:
        st.info("The database is empty. Load something first:")
        st.code("python seed_demo.py      # synthetic demo season\n"
                "python scrape.py         # harvest real matches for your date range",
                language="bash")
        st.stop()
    ...
    comp = field_composition(reg_f if include_dq else reg_f[~reg_f["dq"].astype(bool)],
                             divisions, FREQ[gran])
    cells = class_by_classifier(scores_f, min_n=min_n)
    ...
    with tab_diff:
        if diff.empty:
            ...
            sel = scores_f[scores_f["classifier_code"] == pick]
