...
from pipeline import spectra as spec_mod
...
def render_wr_spectrum(name):
    """JWST/HST spectrum panel for a Wolf-Rayet star: shows a bundled
    spectrum if present, else offers to fetch it from MAST (local only)."""
    star = catalog.WR_BY_NAME.get(name) or {}
    meta = star.get("spectrum")
    spec = spec_mod.load_spectrum(name)
    if spec is None and not meta:
        return  # this star has no spectral data

    st.subheader("Spectrum — JWST / HST")
    if spec is not None:
        st.caption(f"{spec['collection']} · {spec['instrument']} · "
                   f"{spec['filename']} (MAST)")
        st.pyplot(plots.plot_spectrum(
            spec["wavelength"], spec["flux"], spec["wave_unit"],
            spec["flux_unit"],
            title=f"{name} — {spec['collection']} {spec['instrument']}",
            lines=spec_mod.WR_LINES))
        st.caption("Dashed markers: Wolf-Rayet wind emission lines "
                   "(He II, C IV, N V, C III …) within coverage. "
                   "Spectroscopy — a different observatory than the TESS "
                   "photometry.")
    elif meta and _have("astroquery"):
        st.info(f"A {meta['collection']} spectrum exists in MAST for this "
                "star (not bundled).")
        if st.button(f"Fetch {name} spectrum from MAST", key=f"spec_{name}"):
            with st.spinner("Querying MAST + downloading…"):
                try:
                    s = spec_mod.fetch_spectrum(
                        star["ident"], collection=meta["collection"],
                        instrument=meta.get("instrument"))
                    if s:
                        spec_mod.save_spectrum(name, s)
                        st.rerun()
                    else:
                        st.warning("No 1D-spectrum product found.")
                except Exception as exc:
                    st.error(f"Fetch failed: {exc}")
    elif meta:
        st.caption(f"A {meta['collection']} spectrum exists in MAST; run "
                   "locally to fetch it.")
...
def show_wr_unanalyzed(name, star):
    ...
    render_wr_spectrum(name)


def spectrum_mission_view(observatory):
    """
    View for the JWST / HST spectroscopy 'missions': pick a Wolf-Rayet
    star observed by that facility and show its spectrum + portrait.
    Spectrum-focused (these observatories have no light curves), so it
    renders fully and stops.
    """
    stars = [s for s in catalog.WR_STARS
             if (s.get("spectrum") or {}).get("collection") == observatory]
    names = sorted(s["name"] for s in stars)
    if st.session_state.get("target_id") not in names:
        st.session_state.target_id = names[0]
    st.sidebar.selectbox(f"{observatory} target ({len(names)})", names,
                         key="target_id")
    st.sidebar.button("\U0001F3B2 Random", on_click=_pick_random,
                      args=(names,), width="stretch")
    st.sidebar.caption(f"Wolf-Rayet stars with {observatory} spectra")

    name = st.session_state.target_id
    star = catalog.WR_BY_NAME.get(name, {})
    st.title(name)
    st.caption(f"{observatory} spectroscopy · {star.get('note', '')}")

    left, right = st.columns([3, 2])
    with right:
        st.pyplot(plots.render_star(star.get("teff"), star.get("radius"),
                                    name=name))
        st.caption("Simulated from literature Teff — a hot, blue "
                   "Wolf-Rayet star.")
    with left:
        m1, m2 = st.columns(2)
        if star.get("teff"):
        ...
        if star.get("radius"):
            m2.metric("Radius", f"{star['radius']:.1f} R☉")
        if storage.load_target(name) is not None:
            st.caption("This star also has TESS variability — see the "
                       "Wolf-Rayet mission.")
    render_wr_spectrum(name)
    st.stop()
...
# Mission -> object selection. Transit surveys (Kepler/K2/TESS) and the
# Wolf-Rayet variability set come from the DB; JWST/HST are spectroscopy
# observatories added when Wolf-Rayet stars carry spectra from them.
# Hide the Synthetic ground-truth set from the public picker (it stays in
# the DB for validation / the methodology write-up).
missions = [m for m in storage.list_missions() if m != "Synthetic"]
spectral_obs = sorted({s["spectrum"]["collection"] for s in catalog.WR_STARS
                       if s.get("spectrum")})
for observatory in spectral_obs:
    if observatory not in missions:
        missions.append(observatory)
...
st.sidebar.selectbox("Mission", missions, key="mission",
                     help="Kepler / K2 / TESS transit surveys, the "
                          "Wolf-Rayet variability set, or JWST / HST "
...
if mode == "Single target":
    ...
    if st.session_state.mission in ("JWST", "HST"):
        spectrum_mission_view(st.session_state.mission)   # renders + stops
    elif st.session_state.mission == "Wolf-Rayet":
    ...
    # Wolf-Rayet stars can also carry a JWST/HST spectrum.
    if st.session_state.mission == "Wolf-Rayet":
        render_wr_spectrum(data["target_id"])
