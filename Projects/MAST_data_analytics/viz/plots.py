...
def plot_folded_variability(time, flux, period, t0=None,
    ...
    """
    # Fold with the same convention the stored model uses (origin at the
    # first cadence, then centred), so the SHO-PINN overlay lines up with
    # the data rather than sitting half a cycle off.
    t0 = float(np.min(time)) if t0 is None else t0
    phase = ((np.asarray(time, dtype=float) - t0) % period) / period - 0.5
    order = np.argsort(phase)
...
def plot_spectrum(wavelength, flux, wave_unit="um", flux_unit="",
                  title="", lines=None):
    """
    Plot a 1D spectrum (wavelength vs flux) for the JWST/HST Wolf-Rayet
    viewer. `lines` is an optional list of (label, rest_wavelength_um)
    Wolf-Rayet emission lines; those inside the coverage are marked.
    """
    wavelength = np.asarray(wavelength, dtype=float)
    flux = np.asarray(flux, dtype=float)

    fig, ax = plt.subplots(figsize=(9, 4.5))
    ax.plot(wavelength, flux, "-", color="#1f77b4", linewidth=0.9)

    if lines is not None and wavelength.size:
        wmin, wmax = float(wavelength.min()), float(wavelength.max())
        ymax = float(np.nanmax(flux))
        # WR_LINES are tabulated in microns; convert to the spectrum's unit.
        u = (wave_unit or "").lower()
        factor = 1e4 if "ang" in u else 1e3 if "nm" in u else 1.0
        for label, wl_um in lines:
            wl = wl_um * factor
            if wmin <= wl <= wmax:
                ax.axvline(wl, color="#d62728", linestyle="--",
                           linewidth=0.6, alpha=0.5)
                ax.text(wl, ymax, label, rotation=90, va="top", ha="right",
                        fontsize=7, color="#d62728", alpha=0.9)
