...
- `app.py` — Streamlit dashboard: a mission selector (Kepler / K2 /
  TESS) scopes three clearly-labelled object pickers — 📊 analyzed by the
  local PINN, ✅ confirmed planets, and 🟡 candidates still being vetted —
  spanning the whole NASA archive catalogue for that mission. Selecting an
