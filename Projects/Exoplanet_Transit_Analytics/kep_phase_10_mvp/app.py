...
def run_analysis(name, mission, train_pinn, quick):
    """
    ...
    dashboard never pulls them in. Returns (ok, error_message).
    """
    try:
        from pipeline.ingest import fetch_light_curve, get_stellar_params
        from run_pipeline import process_transit
    ...
    try:
        with st.status(f"Analyzing {name}…", expanded=True) as status:
            ...
            if lc is None:
                ...
                return False, f"No {mission} light curve found for {name}."
...
st.sidebar.selectbox("Mission", missions, key="mission",
                     help="Scope targets to a mission (Kepler / K2 / TESS)")
...
if mode == "Single target":
    # Full mission catalogue, split into three clearly-labelled pickers so
    # it's obvious which category each object belongs to: locally analyzed,
    # confirmed planets, and unconfirmed candidates.
    ...
    # Land on an analyzed target so the first view shows real results.
    ...
    # Before the widgets instantiate: keep each picker's stored value valid
    # for the current mission, and make the picker whose category holds the
    # current target reflect it, so the active menu matches the main view.
    ...
    if data is None:
        # Confirmed or candidate object we haven't run yet.
